...
export default function CardsManagement() {
  ...
  const handleDeleteCard = async (cardId) => {
    ...
    try {
      const res = await axiosSecure.delete(`/api/cards/${cardId}`);
      if (res.status === 200 || res.data.deletedCount > 0 || res.data.success) {
  ...
  const handleModalSubmit = async (cardData) => {
    try {
      ...
      Swal.fire(
        "Success",
        `Card ${modal.type === "add" ? "added" : "updated"} successfully`,
        "success"
      );
    } catch (err) {
      console.error(err?.response?.data || err.message);
      Swal.fire(
        ...
        `Failed to ${modal.type === "add" ? "add" : "update"} card`,
        "error"
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 hover:shadow-2xl transition-all duration-500">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            className="bg-primary cursor-pointer hover:bg-primary text-white px-6 py-3 rounded-lg font-medium flex items-center gap-2 shadow-lg"
          >
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-8">
                      ...
                      <div
                        className="relative"
                        ref={openDropdown === card._id ? dropdownRef : null}
                      >
                        <button
                          onClick={() => handleDropdownClick(card._id)}
                          className="text-white hover:bg-white/20 p-1 rounded cursor-pointer transition-colors"
                        >
                        ...
                        {openDropdown === card._id && (
                          <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2 z-50 border border-gray-100">
                            <button
                              onClick={() =>
                                Swal.fire({
                                  title: `<div style="font-size:20px; font-weight:600; color:#333;">${card.cardName}</div>`,
                                  html: `
        <div style="
          text-align:left;
          font-size:15px;
          ...
          margin-top:10px;
        ">
          <div><strong>💳 Card Number:</strong> ${card.cardNumber || "N/A"}</div>
          <div><strong>📅 Expiry Date:</strong> ${card.expiryDate || "N/A"}</div>
          <div><strong>🏦 Card Type:</strong> ${card.cardType || "N/A"}</div>
          <div><strong>💰 Balance:</strong> $${Number(card.balance || 0).toLocaleString("en-US", {
      ...
      `,
                              ...
                              className="w-full px-4 py-2 text-left text-gray-700 cursor-pointer hover:bg-gray-50 flex items-center gap-3"
                            >
                            ...
                            <button
                              onClick={() =>
                                setModal({ open: true, type: "edit", card })
                              }
                              className="w-full px-4 py-2 text-left text-gray-700 cursor-pointer hover:bg-gray-50 flex items-center gap-3"
                            >
                            ...
                            <button
                              ...
                              className="w-full px-4 py-2 text-left text-red-600 cursor-pointer hover:bg-red-50 flex items-center gap-3"
                            >
...
function CardForm({ card, onSubmit, onCancel }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!cardName || (!cardNumber && !card?.meta?.last4) || !expiryDate) {
      return Swal.fire(
        "Error",
        "Please fill required fields (card name, number, expiry)",
        "error"
      );
    }
    const payload = { cardName, expiryDate, cardType, balance, bank };
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <input
        type="text"
        placeholder={card ? "New Card Number (optional)" : "Card Number"}
        value={cardNumber}
